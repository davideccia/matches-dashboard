<template>
  <div class="relative h-full">
    <UBadge
      ...
      variant="solid"
      size="sm"
      class="absolute -top-2.5 left-4 z-10"
    >
      {{ t('match.current') }}
    </UBadge>
    ...
    <article
      ...
      :class="[
        ...
        current ? 'ring-2 ring-primary ring-offset-2 ring-offset-default' : '',
      ]"
    >
...
const props = defineProps<{
  ...
  /** Evidenzia la card come "match corrente" (usato per il prossimo match programmato quando nessuno è in corso). */
  current?: boolean
}>()
