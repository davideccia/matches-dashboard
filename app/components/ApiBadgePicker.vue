<template>
  ...
  >
        ...
        <div class="flex flex-wrap gap-2">
          ...
          >
            <span class="flex items-center gap-1">
              <UIcon v-if="isSelected(item)" name="i-mdi-check" class="size-3.5 shrink-0" />
    ...
    <div class="flex items-center justify-between gap-2 border-t border-default px-2 py-1.5 text-xs text-muted">
      <span>
        {{ modelValue.length }}<template v-if="max"> / {{ max }}</template>
        {{ t('common.selected') }}
        <span v-if="isMaxReached" class="text-warning">— {{ t('common.maxSelectionReached') }}</span>
      </span>
      <UButton
        v-if="!isDisabled"
        type="button"
        ...
        :leading-icon="allLoadedSelected ? 'i-mdi-checkbox-multiple-blank-outline' : 'i-mdi-checkbox-multiple-marked-outline'"
        @click="toggleAll"
      >
...
const labelOf = (item: Item) => String(item[props.labelKey] ?? item[props.valueKey] ?? '')
...
const canToggleAll = computed(() => items.value.length > 0 || modelValue.value.length > 0)
...
const displayItems = computed<Item[]>(() => {
  ...
  const rest = items.value.filter(item => !modelValue.value.includes(idOf(item)))
...
function cacheSelected(fetched: Item[]) {
  const missing = fetched.filter(
    item => modelValue.value.includes(idOf(item)) && !selectedCache.value.has(idOf(item)),
  )
  if (missing.length === 0) { return }
  const next = new Map(selectedCache.value)
  for (const item of missing) { next.set(idOf(item), item) }
...
// ── Selection ──────────────────────────────────────────────────────────────
function emitChange() {
  emit('change', modelValue.value.map(id => selectedCache.value.get(id) ?? { [props.valueKey]: id } as Item))
  emitFormChange()
}
...
function toggle(item: Item) {
  ...
  if (!selected && isMaxReached.value) { return }
...
function selectAllLoaded() {
  const missing = items.value.filter(item => !modelValue.value.includes(idOf(item)))
  const room = props.max === undefined ? missing.length : props.max - modelValue.value.length
  const toAdd = missing.slice(0, Math.max(room, 0))
  if (toAdd.length === 0) { return }

  const cache = new Map(selectedCache.value)
  for (const item of toAdd) { cache.set(idOf(item), item) }
...
watch(() => props.queryParams, () => { reset() }, { deep: true })

// ── initialItems arriva in modifica dopo il GET del parent ─────────────────
watch(() => props.initialItems, (list) => {
  if (list?.length) { cacheSelected(list) }
}, { deep: true })
