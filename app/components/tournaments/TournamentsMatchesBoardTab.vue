<template>
  <div class="flex flex-col h-full overflow-hidden">
    <div class="shrink-0 p-6 pb-0">
      <OrphanRegistrationsAlert :items="matchmakingIssues" :skeleton="orphanLoading" />
    ...
    <div class="shrink-0 border-b border-muted px-6 py-3 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
      ...
      <UBadge v-if="total > 0" variant="soft" color="neutral" size="md">
        {{ total }}
      </UBadge>
    </div>
...
async function refreshMatchmakingIssues() {
  ...
  try {
    const tournament = await api.get<{ data: { matchmaking_issues: MatchmakingIssue[] | null } }>(`/api/admin/tournaments/${props.tournamentId}`)
...
const { data, refresh, status } = useLazyAsyncData(
  `matches-board-${props.tournamentId}`,
  () => {
    return api.get<MatchRecordesResponse>(`/api/admin/tournaments/${props.tournamentId}/match_records?with=tournament,red_corner,blue_corner,winner,weight_category,discipline`, {
      page: 1,
...
async function generateMatches() {
  ...
  try {
    await api.post(`/api/admin/tournaments/${props.tournamentId}/match_records/generate`)
